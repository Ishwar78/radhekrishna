...
export default function Dresses() {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (data.success || data.products) {
          const filtered = (data.products || []).filter((p: any) => p.subcategory === "Dresses");
  ...
  if (isLoading) {
    return (
      <CollectionLayout
        title="Loading..."
        metaTitle="Dresses"
        metaDescription="Explore our dresses collection"
        products={[]}
        filterCategories={[]}
  ...
  return (
    <CollectionLayout
      ...
      tagline="Timeless silhouettes for every occasion"
      metaTitle="Dresses | Vasstra"
      metaDescription="Explore our dresses collection"
      products={products}
      filterCategories={[]}
      heroBg="bg-gradient-to-b from-secondary/10 to-background"
